 ...
 * @param {Array} props.schedules - Existing schedules for this report
 * @param {Array} props.exportFormats - Available export formats
 * @param {Array} props.recipients - Available recipients for scheduled reports
 * @param {Function} props.onAddSchedule - Function called when a schedule is added
 * @param {Function} props.onUpdateSchedule - Function called when a schedule is updated
 * @param {Function} props.onDeleteSchedule - Function called when a schedule is deleted
...
const ReportScheduleDialog = ({
  ...
  const handleRecipientToggle = (recipientId) => {
    ...
    if (errors.recipientIds) {
      setErrors(prev => ({ ...prev, recipientIds: '' }));
    }
  };
  ...
  const getDeliveryMethodText = (schedule) => {
    const methods = [];
  ...
  return (
    ...
    >
                    ...
                    <ListItemText
                      ...
                      secondary={
                        <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1, mt: 0.5 }}>
                          ...
                          />
                          {schedule.includeEmail && schedule.recipientIds.length > 0 && (
                            <Typography variant="caption" color="text.secondary">
                              {schedule.recipientIds.length} recipient(s)
                            </Typography>
                          )}
                        </Box>
                      }
      ...
      <DialogActions sx={{ p: 2 }}>
        ...
        >
          {loading ? <CircularProgress size={24} /> : (isEditing ? 'Update Schedule' : 'Add Schedule')}
        </Button>
      </DialogActions>
    </Dialog>
  );

...
const generateTimeSeriesData = (id, months = 12) => {
...
const timeSeriesDataCache = {};

// Get time series data for a node, with caching
const getTimeSeriesData = (nodeId) => {
...
const getAdditionalMetrics = (nodeId) => {
 ...
 */
const InteractiveDrillDownExplorer = ({
  data = sampleRevenueData,
  title = "Revenue Explorer",
  isLoading = false
}) => {
  const theme = useTheme();
  const [currentPath, setCurrentPath] = useState([data.id]);
  const [activeIndex, setActiveIndex] = useState(0);
  const [viewMode, setViewMode] = useState('chart'); // 'chart' or 'table'
  const [chartType, setChartType] = useState('pie'); // 'pie', 'bar', 'treemap'
  const [detailChartType, setDetailChartType] = useState('area'); // 'line', 'area', 'bar'
  ...
  const breadcrumbItems = currentPath.map((id, index) => {
    let node = data;
  ...
  const handleDetailChartTypeChange = (event) => {
    setDetailChartType(event.target.value);
  };
      ...
      <ResponsiveContainer width="100%" height={500}>
        <PieChart>
          <Pie
            ...
            dataKey="value"
            nameKey="name"
            onClick={(data) => data.children ? handleDrillDown(data.id) : handleOpenDetail(data)}

...
import {
  ...
  Metric,
  InsightType
} from '../../types/contextualExplanation';
 ...
 */
const ContextualExplanation: React.FC<ContextualExplanationProps> = ({
  metric,
  data,
  insightType = 'trend',
  isLoading = false,
  ...
  const generateExplanation = async (): Promise<void> => {
    try {
      const params = {
        metric: metric,
        data: data,
      ...
      const explanations: Record<InsightType, Explanation> = {
        trend: {
  ...
  const getTrendDirection = (data: DataPoint[]): string => {
  ...
  const getPerformanceInsight = (data: DataPoint[], metric: Metric): string => {
  ...
  const getBusinessImpact = (data: DataPoint[], metric: Metric): string => {
    const direction = getTrendDirection(data);
  ...
  const getRecommendation = (data: DataPoint[], metric: Metric, type: InsightType): string => {
  ...
  const getAnomalyCount = (data: DataPoint[]): number => {
  ...
  const getAnomalyDates = (data: DataPoint[]): string => {
  ...
  const getDeviationAmount = (data: DataPoint[]): number => {
  ...
  const getAnomalyInsight = (data: DataPoint[], metric: Metric): string => {
  ...
  const getAnomalyImpact = (data: DataPoint[], metric: Metric): string => {
    const anomalies = data?.filter(item => item.isAnomaly) || [];
  ...
  const getForecastDirection = (data: DataPoint[]): string => {
  ...
  const getForecastValue = (data: DataPoint[]): number => {
  ...
  const getForecastChange = (data: DataPoint[]): number => {
  ...
  const getPredictionConfidence = (data: DataPoint[]): string => {
  ...
  const getVariance = (data: DataPoint[]): number => {
    if (!data || data.length < 2) return 0;
  ...
  const getComparisonResult = (data: DataPoint[], metric: Metric): string => {
  ...
  const getPerformanceGap = (data: DataPoint[], metric: Metric): number => {
    const avg = calculateAverage(data);
    const industryAvg = avg * (1 + (Math.random() * 0.4 - 0.2)); // +/- 20% of your average
  ...
  const getComparisonDirection = (data: DataPoint[]): string => {
    const avg = calculateAverage(data);
    const industryAvg = avg * (1 + (Math.random() * 0.4 - 0.2)); // +/- 20% of your average
  ...
  const getPerformanceTier = (data: DataPoint[]): string => {
  ...
  const getCompetitiveInsight = (data: DataPoint[], metric: Metric): string => {
  ...
  const getCorrelationCoefficient = (data: DataPoint[]): number => {
    // Simulate correlation coefficient (-1 to 1)
    return Math.random() * 1.6 - 0.8; // between -0.8 and 0.8
  };

  const getCorrelationStrength = (data: DataPoint[]): string => {
  ...
  const getCorrelationMeaning = (data: DataPoint[], metric: Metric): string => {
  ...
  const getCorrelationInsight = (data: DataPoint[], metric: Metric): string => {
  ...
  const getInsightIcon = (): React.ReactNode => {
    switch (insightType) {
      case 'trend':
        return <TimelineIcon />;
      case 'anomaly':

...
const InsightPanel = ({
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
                        ...
                        <Box>
                          <Button 
                            size="small" 
                            variant="outlined"
                            onClick={() => onInsightAction(issue.id, 'fix')}
                            sx={{ mr: 1 }}
                          >
                            Fix Issue
                          </Button>
  ...
  // Render insight stats summary
  const renderInsightStats = () => {
    const totalAnomalies = insights.anomalies.length;
    const highSeverityAnomalies = insights.anomalies.filter(a => a.severity === 'high').length;
    const positiveRecommendations = insights.recommendations.filter(r => r.impact === 'high').length;
      ...
      <Box sx={{ display: 'flex', mb: 3 }}>
        <Card sx={{ flexGrow: 1, mr: 2 }}>
          <CardContent sx={{ display: 'flex', alignItems: 'center', p: 2, '&:last-child': { pb: 2 } }}>
            ...
            <Typography variant="body2">
              {totalAnomalies === 0 ? 'No anomalies detected' : (
                <>
                  <strong>{totalAnomalies}</strong> anomalies detected
                  {highSeverityAnomalies > 0 && (
                    <>, <strong>{highSeverityAnomalies}</strong> high severity</>
                  )}
                </>
              )}
            </Typography>
        ...
        <Card sx={{ flexGrow: 1 }}>
          <CardContent sx={{ display: 'flex', alignItems: 'center', p: 2, '&:last-child': { pb: 2 } }}>
            ...
            <Typography variant="body2">
              <strong>{insights.recommendations.length}</strong> recommendations
              {positiveRecommendations > 0 && (
                <>, <strong>{positiveRecommendations}</strong> high impact</>
              )}
            </Typography>
          </CardContent>
        </Card>

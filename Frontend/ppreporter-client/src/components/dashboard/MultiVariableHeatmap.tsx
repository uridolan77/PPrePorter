...
import {
  ...
  DimensionOptions,
  ValueMetricOptions
} from '../../types/multiVariableHeatmap';
 ...
 */
const MultiVariableHeatmap: React.FC<MultiVariableHeatmapProps> = ({
  height = 600,
  isLoading = false,
  timeFrame = '30d'
}) => {
  ...
  const [dataType, setDataType] = useState<string>('player_activity');
  const [xAxis, setXAxis] = useState<string>('day_of_week');
  const [yAxis, setYAxis] = useState<string>('hour_of_day');
  const [valueMetric, setValueMetric] = useState<string>('player_count');
  ...
  const [colorPalette, setColorPalette] = useState<string>('blues');
  ...
  const valueMetricOptions: ValueMetricOptions = {
    player_activity: [
  ...
  const colorPaletteOptions: Option[] = [
  ...
  const handleDataTypeChange = (event: SelectChangeEvent): void => {
  ...
  const handleXAxisChange = (event: SelectChangeEvent): void => {
    setXAxis(event.target.value);
  };

  const handleYAxisChange = (event: SelectChangeEvent): void => {
    setYAxis(event.target.value);
  };

  const handleValueMetricChange = (event: SelectChangeEvent): void => {
    setValueMetric(event.target.value);
  };

  const handleCellSizeChange = (_event: Event, newValue: number | number[]): void => {
    setCellSize(newValue as number);
  };

  const handleShowValuesChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    setShowValues(event.target.checked);
  };

  const handleReverseColorsChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    setReverseColors(event.target.checked);
  };

  const handleColorPaletteChange = (event: SelectChangeEvent): void => {
    setColorPalette(event.target.value);
  };

  const handleSearchFilterChange = (event: ChangeEvent<HTMLInputElement>): void => {
  ...
  const generateAccessibleDescription = (): string => {
    ...
    heatmapData.data.forEach((row: Record<string, any>) => {
  ...
  const getFilteredData = (): HeatmapDataRow[] => {
    ...
    return heatmapData.data.filter((row: Record<string, any>) => {
        ...
        {filteredData.length === 0 ? (
          ...
          >
            <ResponsiveHeatMap
              ...
              valueFormat={v => formatValue(v as number, valueMetric)}
              xOuterPadding={0.05}
              yOuterPadding={0.05}
              axisTop={{
                tickSize: 5,
              ...
              legends={[
                {
                  ...
                  tickSpacing: 4,
                  tickOverlap: false,
                  tickFormat: v => formatValue(v as number, valueMetric),

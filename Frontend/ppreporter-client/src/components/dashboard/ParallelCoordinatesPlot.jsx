...
const ParallelCoordinatesPlot = ({
  ...
  const handleColorDimensionChange = (event) => {
    setColorDimension(event.target.value);
  };
  
  // Handle sort change
  const handleSort = (dimensionId) => {
  ...
  const handleFilterChange = (dimensionId, range) => {
    setFilters(prev => ({
      ...prev,
      [dimensionId]: range
    }));
  ...
  const getChartDimensions = () => {
    return selectedDimensions.map(dimId => {
      ...
      const values = filteredData.map(item => item[dimId]).filter(val => val !== null && val !== undefined);
      ...
      return {
        ...
        domain: [min, max],
        tickFormatter: (value) => formatValue(value, dimension.format),
        type: 'number'
      };
    }).filter(Boolean);
  };
  ...
  const getColorScale = () => {
    ...
    if (dimension && dimension.type === 'categorical') {
      ...
      categories.forEach((category, index) => {
        colorMap[category] = categoryColors[index % categoryColors.length];
      });
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Grid container spacing={2}>
          {selectedDimensions.map(dimId => {
            ...
            const values = data.map(item => item[dimId]).filter(val => val !== null && val !== undefined);
            const min = Math.min(...values);
            const max = Math.max(...values);
              ...
              <Grid item xs={12} sm={6} md={4} key={dimId}>
                <Box sx={{ px: 1 }}>
                  ...
                  <Slider
                    ...
                    max={max}
                    step={(max - min) / 100}
                    onChange={(e, newValue) => handleFilterChange(dimId, { min: newValue[0], max: newValue[1] })}
...
// Make sure Grid component is imported

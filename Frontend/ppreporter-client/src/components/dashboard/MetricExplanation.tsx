...
import { MetricExplanationProps, ExplanationData } from '../../types/metricExplanation';
...
const MetricExplanation: React.FC<MetricExplanationProps> = ({
  ...
  const fetchExplanation = async (): Promise<void> => {
    ...
    try {
      // Convert userContext to a string parameter (JSON string)
      const contextParam = JSON.stringify(userContext);
      const data = await dashboardAnalyticsService.getMetricExplanation(metricId, contextParam);
      // Ensure data conforms to ExplanationData type
      setExplanation(data as ExplanationData);
    } catch (err) {

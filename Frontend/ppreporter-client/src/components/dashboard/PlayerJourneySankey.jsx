...
const PlayerJourneySankey = ({ 
  ...
  const { playerJourneyData, playerJourneyLoading, playerJourneyError } = useSelector(state => state.dashboard);
  
  const [journeyType, setJourneyType] = useState('acquisition_to_game');
  ...
  const getNodeColor = (node) => {
    ...
    switch (colorMode) {
      ...
      case 'value':
        ...
        const maxValue = Math.max(...(playerJourneyData?.nodes || []).map(n => n.value));
        const intensity = node.value / maxValue;
        return `rgba(33, 150, 243, ${0.5 + (intensity * 0.5)})`;
      case 'source':
        return node.sourceNodes?.length > 0 ? theme.palette.primary.main : theme.palette.secondary.main;
      case 'target':
        return node.targetNodes?.length > 0 ? theme.palette.success.main : theme.palette.warning.main;
      default:
  ...
  const getLinkColor = (link) => {
    switch (colorMode) {
      case 'category':
        ...
        return { from: getNodeColor(link.source), to: getNodeColor(link.target) };
      case 'value':
        // Higher value links get deeper color
        const maxValue = Math.max(...(playerJourneyData?.links || []).map(l => l.value));
        const intensity = link.value / maxValue;
        return `rgba(33, 150, 243, ${0.2 + (intensity * 0.6)})`;
      case 'source':
        return getNodeColor(link.source);
      case 'target':
        return getNodeColor(link.target);
      default:
        return theme.palette.primary.main;
    }
  };
  ...
  const getLinkTooltip = (link) => {
    ...
    const percentage = ((link.value / link.source.value) * 100).toFixed(1);
  ...
  const generateAccessibleDescription = () => {
    ...
    const topLinks = [...playerJourneyData.links]
      .sort((a, b) => b.value - a.value)
      .slice(0, 3)
      .map(link => `${getNodeLabel(link.source)} to ${getNodeLabel(link.target)} with ${link.value.toLocaleString()} players`);
          ...
          {nodeHovered ? (
            <Typography variant="body2">
              {getNodeLabel({ id: nodeHovered })} accounts for {
                playerJourneyData.nodes.find(n => n.id === nodeHovered)?.value.toLocaleString()

# PowerShell script to replace services with fixed implementations

# Location for the fixed implementations
$servicesDir = "c:\dev\PPrePorter\Backend\PPrePorter.SemanticLayer\Services"

# Move DataModelService to a backup
$dataModelServicePath = Join-Path $servicesDir "DataModelService.cs"
if (Test-Path $dataModelServicePath) {
    Rename-Item -Path $dataModelServicePath -NewName "DataModelService.cs.old" -Force
    Write-Host "Backed up DataModelService.cs to DataModelService.cs.old"
}

# Create the new DataModelService
$newDataModelServiceContent = @'
...
Set-Content -Path $dataModelServicePath -Value $newDataModelServiceContent -Force
Write-Host "Created new DataModelService.cs"

    ...
    public partial class DailyActionGameController
    {
            ...
            [FromQuery] long? gameId = null)
        {
            try
            {
                ...
                {
                    dailyActionGames = await _dailyActionGameService.GetDailyActionGamesByPlayerIdAsync(playerId.Value);
                    // Filter by date range
                    dailyActionGames = dailyActionGames.Where(g => g.GameDate >= start && g.GameDate <= end);
                }
                else if (gameId.HasValue)
                {
                    dailyActionGames = await _dailyActionGameService.GetDailyActionGamesByGameIdAsync(gameId.Value);
                    // Filter by date range
                    dailyActionGames = dailyActionGames.Where(g => g.GameDate >= start && g.GameDate <= end);
                }
                else
                {
                    // Get all games and filter by date range
                    dailyActionGames = await _dailyActionGameService.GetAllDailyActionGamesAsync();
                    dailyActionGames = dailyActionGames.Where(g => g.GameDate >= start && g.GameDate <= end);
                }
